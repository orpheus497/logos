...
class TestGetActiveWorkflow:
    ...
    def test_returns_none_when_no_active(self, tmp_path):
        ...
        (tracking / "diamond_workflow.md").write_text(
            "# Diamond\n**Status:** COMPLETE\n"
        )
    ...
    def test_returns_in_progress_workflow(self, tmp_path):
        tracking = tmp_path / "WORKFLOW_TRACKING"
        tracking.mkdir(parents=True)
        (tracking / "funnel_workflow.md").write_text(
            "# Funnel\n**Status:** IN PROGRESS\n- [ ] **Step 1 (B6)**\n"
        )
        result = get_active_workflow(tmp_path)
        assert result is not None
...
class TestEndOfTaskProtocolPresence:
    ...
    def test_all_daedelus_agents_have_end_of_task(self, daedelus_prompts):
        ...
        for agent_key, prompt in daedelus_prompts.items():
            assert "END-OF-TASK PROTOCOL" in prompt, (
                f"Daedelus agent {agent_key} missing END-OF-TASK protocol"
            )
    ...
    def test_all_deus_agents_have_end_of_task(self, deus_prompts):
        ...
        for agent_key, prompt in deus_prompts.items():
            assert "END-OF-TASK PROTOCOL" in prompt, (
                f"DEUS agent {agent_key} missing END-OF-TASK protocol"
            )
    ...
    def test_daedelus_end_of_task_contains_required_steps(self, daedelus_prompts):
        ...
        for agent_key, prompt in daedelus_prompts.items():
            assert "Step 1: Update .devdocs/DEV_STATE.md" in prompt, (
                f"Daedelus {agent_key} missing Step 1"
            )
            assert "Step 2: Update Your Agent Log" in prompt, (
                f"Daedelus {agent_key} missing Step 2"
            )
            assert "Step 3: Recommend Next Agent" in prompt, (
                f"Daedelus {agent_key} missing Step 3"
            )
            assert "Step 4: Report Completion" in prompt, (
                f"Daedelus {agent_key} missing Step 4"
            )
    ...
    def test_deus_end_of_task_contains_required_steps(self, deus_prompts):
        ...
        for agent_key, prompt in deus_prompts.items():
            assert "Step 1: Update .devdocs/DEV_STATE.md" in prompt, (
                f"DEUS {agent_key} missing Step 1"
            )
            assert "Step 2: Update Your Agent Log" in prompt, (
                f"DEUS {agent_key} missing Step 2"
            )
            assert "Step 3: Recommend Next Agent" in prompt, (
                f"DEUS {agent_key} missing Step 3"
            )
            assert "Step 4: Report Completion" in prompt, (
                f"DEUS {agent_key} missing Step 4"
            )
    ...
    def test_daedelus_end_of_task_has_agent_specific_recommendations(self, daedelus_prompts):
        ...
        for agent_key, prompt in daedelus_prompts.items():
            assert "Recommend" in prompt or "recommend" in prompt, (
                f"Daedelus {agent_key} missing recommendations"
            )
    ...
    def test_deus_end_of_task_has_agent_specific_recommendations(self, deus_prompts):
        ...
        for agent_key, prompt in deus_prompts.items():
            assert "Recommend" in prompt or "recommend" in prompt, (
                f"DEUS {agent_key} missing recommendations"
            )
    ...
    def test_end_of_task_count_matches_agent_count(self, daedelus_prompts, deus_prompts):
        ...
        daedelus_count = sum(
            1 for p in daedelus_prompts.values() if "END-OF-TASK PROTOCOL" in p
        )
        deus_count = sum(
            1 for p in deus_prompts.values() if "END-OF-TASK PROTOCOL" in p
        )
